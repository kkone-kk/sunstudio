...
export const generateVideo = async (
    ...
    // 2. Handle First & Last Frame Logic (FrameWeaver)
    // If we are in FIRST_LAST_FRAME mode, we expect 2 images in referenceImages (or 1 in inputs.image + 1 in referenceImages)
    if (options.generationMode === 'FIRST_LAST_FRAME' && referenceImages && referenceImages.length >= 2) {
        // According to strategy:
        // Index 0 = Start Frame
        // Index 1 = Last Frame
        try {
            // Force model to Veo Fast if not already (it supports keyframing reliably)
            // But we respect user choice if they chose Pro (Pro also supports it)
            // model = 'veo-3.1-fast-generate-preview'; 
            
            // Set Start Frame (image)
            const startCompat = await convertImageToCompatibleFormat(referenceImages[0]);
            inputs.image = { imageBytes: startCompat.data, mimeType: startCompat.mimeType };
            finalInputImageBase64 = startCompat.fullDataUri;

            // Set Last Frame (config.lastFrame)
            const endCompat = await convertImageToCompatibleFormat(referenceImages[1]);
            config.lastFrame = { imageBytes: endCompat.data, mimeType: endCompat.mimeType };
            
        } catch (e) {
            console.error("FrameWeaver: Failed to process start/end frames", e);
            throw new Error("Failed to process start/end frames for video generation.");
        }
    }
    // 3. Handle General Reference Images (for CharacterRef, etc, but NOT First/Last frame mode)
    else if (referenceImages && referenceImages.length > 0 && model === 'veo-3.1-generate-preview') {
         const refsPayload = [];
    ...
    // 4. Handle Video Input (e.g. for edit/continuation)
    if (videoInput) {
    ...
    try {
        ...
        try {
            ...
            // Only use the START image for fallback to avoid confusion
